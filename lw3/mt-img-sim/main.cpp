...
void ProcessFile(const Image& queryImage,
	...
	std::mutex& resultsMutex)
{
	try
	{
		...
		globalResults.push_back({ mseValue, filePath });
	}
...
void FindSimilarImages(const Config& config)
{
	...
	boost::asio::thread_pool pool(config.numThreads);
...
int main(const int argc, char** argv)
{
	...
	try
	{
		...
		FindSimilarImages(config);

		const auto end = std::chrono::high_resolution_clock::now();
		const auto duration = std::chrono::duration_cast<std::chrono::milliseconds>(end - start);
		std::cout << "Время выполнения для " << config.numThreads << " потоков: " << duration.count() << " мс" << std::endl;
	}
...
// {
// 	// TODO: разделить массив файлов сразу. проверить, что так быстрее -- Локальность кэша
